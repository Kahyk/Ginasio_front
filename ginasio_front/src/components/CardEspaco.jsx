...
const CardEspaco = ({ espaco }) => {
  // define as cores das tags baseado na categoria
  const getCorCategoria = (categoria) => {
    switch (categoria) {
      case 'Quadra Poliesportiva': return { bg: '#e7f1ff', text: '#0d6efd' };
      case 'Academia': return { bg: '#f3e8ff', text: '#6f42c1' };
      case 'Piscina': return { bg: '#cff4fc', text: '#0dcaf0' };
      case 'Estádio de Atletismo': return { bg: '#ffebd5', text: '#fd7e14' };
      case 'Tênis': return { bg: '#d1e7dd', text: '#198754' };
      case 'Beach Tênis': return { bg: '#fff3cd', text: '#ffc107' };
      case 'Área Verde': return { bg: '#e8f5e9', text: '#28a745' };
    ...
    <div className="card-unifor p-4 mb-3 border h-100 d-flex flex-column">
      ...
      <div className="mb-3 flex-grow-1">
        ...
        <div className={`d-flex align-items-center gap-2 small mb-2 ${espaco.coberto ? 'text-success' : 'text-warning'}`}>
          ...
          <span>{espaco.coberto ? 'Espaço coberto' : 'Área aberta (sujeita a clima)'}</span>
        </div>
      ...
      <div>
        ...
        <div className="d-flex flex-wrap gap-2">
          {espaco.comodidades.map((item, index) => (
            <span key={index} className="border rounded px-2 py-1 text-body bg-transparent" style={{ fontSize: '11px' }}>
              {item}
            </span>
          ))}
        </div>
